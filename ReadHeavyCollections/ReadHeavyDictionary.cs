...
public sealed class ReadHeavyDictionary<TKey, TValue> : ICollection<KeyValuePair<TKey, TValue>>, IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable, IDictionary<TKey, TValue>, IReadOnlyCollection<KeyValuePair<TKey, TValue>>, IReadOnlyDictionary<TKey, TValue>, ICollection, IDictionary, ISerializable, IDeserializationCallback where TKey : notnull
{
    ...
    /// </summary>
    /// <param name="items">A key/value enumerable to add to the dictionary.</param>
    ...
    public bool ContainsValue(TValue value)
    {
        ...
        foreach (var v in _frozenDictionary.Values)
        {
    ...
    private void Freeze()
    {
        var frozen = (_isComparerSet)
            ? _dictionary.ToFrozenDictionary(_comparer)
            : _dictionary.ToFrozenDictionary();

        Volatile.Write(ref _frozenDictionary, frozen);
    }
    ...
    IDictionaryEnumerator IDictionary.GetEnumerator() => ((IDictionary)_frozenDictionary).GetEnumerator();
